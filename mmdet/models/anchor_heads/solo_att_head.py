...
class SOLOAttHead(nn.Module):
    ...
    def loss(self,
        ...
        cate_labels = [torch.stack(list(cate_label_level), 0) for cate_label_level in zip(*cate_label_list)]
        cate_labels = [
    ...
    def solo_target_single(self,
        ...
        for (lower_bound, upper_bound), stride, featmap_size \
            ...
            for seg_mask, gt_label, gt_bbox in zip(gt_masks, gt_labels, gt_bboxes):
                ...
                if attention_raw.shape[0] and attention_raw.shape[1]:
                    ...
                    attention = torch.zeros([self.attention_size,self.attention_size], dtype=torch.uint8, device=device)
